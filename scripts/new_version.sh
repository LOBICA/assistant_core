...
echo "Expecting CHANGELOG.md to contain an entry for $NEW_VERSION"
if ! grep -q "## $NEW_VERSION" CHANGELOG.md; then
	echo "Error: CHANGELOG.md does not contain a header '## $NEW_VERSION'. Please add release notes before running this script." >&2
	exit 1
fi
